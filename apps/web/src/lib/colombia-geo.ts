/**
 * Departamentos y municipios de Colombia (listado DANE completo: 33
 * departamentos, 1104 municipios). Bogotá D.C. se modela como departamento
 * propio. Generado desde marcovega/colombia-json y ordenado alfabéticamente.
 * Alimenta el selector en cascada departamento → ciudad.
 */
export const COLOMBIA: { department: string; cities: string[] }[] = [
  ...
  { department: 'Vaupés', cities: ['Carurú', 'Mitú', 'Taraira'] },
  {
    department: 'Vichada',
...
export const DEPARTMENTS = COLOMBIA.map((d) => d.department);

export function citiesOf(department: string): string[] {
  return COLOMBIA.find((d) => d.department === department)?.cities ?? [];
}
