...
export const creatorProfile = pgTable('creator_profile', {
  ...
  contentStyles: text('content_styles').array().notNull().default(sql`'{}'`),
  formats: text('formats').array().notNull().default(sql`'{}'`),
  declaredAudience: jsonb('declared_audience').notNull().default({}),
...
export const organization = pgTable('organization', {
  ...
  sectors: text('sectors').array().notNull().default(sql`'{}'`),
  customSector: varchar('custom_sector', { length: 80 }),
